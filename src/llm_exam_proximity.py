...
"""
LLM--student answer proximity analysis for written exams.

This script compares student answers with reference answers generated by several
large language models (LLMs). It uses a transparent TF-IDF representation with
unigrams and bigrams, cosine similarity in the original TF-IDF space, and t-SNE
only as a two-dimensional visualization layer.

The method is intended for exploratory visualization and pedagogical analysis.
It is not a formal detector of misconduct and should not be used as the sole
basis for individual accusations.
"""
...
SPECIAL_STUDENT_NAMES = {"META", "ALIBABA", "IBM", "KIMI", "CHEAT"}
...
_R_MARK = re.compile(r"(?:^|\n)\s*R\s*([1-4])\s*\b", flags=re.IGNORECASE)
...
def split_variants(text: str) -> List[Tuple[str, str]]:
    """
    Return a list of (variant_id, variant_text).

    If labels such as 'COPIE A' are found, the text is split into COPIE A/B/C/D
    blocks. Otherwise the whole file is treated as a single variant called 'ALL'.
    """
...
def extract_R_blocks(text: str) -> Dict[int, List[str]]:
    """Return a dictionary {1:[...], 2:[...], 3:[...], 4:[...]} from R1..R4 blocks."""
    text = clean_text(text)
...
def normalize_student_name(filename_stem: str) -> str:
    """Remove common prefixes and harmonize special seeded/test names."""
    sid = re.sub(r"^UE1_partiel[-_ ]*", "", filename_stem, flags=re.IGNORECASE)
    sid = clean_text(sid)
    sid_upper = sid.upper()

    for special_name in SPECIAL_STUDENT_NAMES:
        if sid_upper.startswith(special_name):
            return special_name

    return sid


def build_anonymization_map(students_folder: Path, output_dir: Path, save_map: bool = True):
    """
    Build a reproducible mapping from original student file names to E01, E02, ...

    Seeded/test names listed in SPECIAL_STUDENT_NAMES are preserved.
    """
    student_files = sorted(
        [p for p in students_folder.iterdir() if p.is_file() and p.suffix.lower() == ".txt"],
    ...
    for path in student_files:
        original_id = normalize_student_name(path.stem)
        original_id_upper = original_id.upper()

        if original_id_upper in SPECIAL_STUDENT_NAMES:
            anonymous_id = original_id_upper
        else:
            anonymous_id = f"E{anon_counter:02d}"
            anon_counter += 1

        anon_map[path.name] = anonymous_id
        rows.append({"filename": path.name, "original_id": original_id, "anonymous_id": anonymous_id})

    map_df = pd.DataFrame(rows)

    if save_map:
        map_df.to_csv(output_dir / "student_anonymization_map.csv", index=False, encoding="utf-8")

    return anon_map, map_df
...
def build_ia_files(data_dir: Path, n_llm_responses: int, models: List[str]) -> Dict[str, Path]:
    """Build the dictionary of LLM reference-answer files from the requested corpus size."""
    ia_files = {}
    for model in models:
        prefix = MODEL_FILE_PREFIX[model]
        ia_files[model] = data_dir / f"{prefix}{n_llm_responses}.txt"
    return ia_files
...
def build_R_corpus(
    ...
    """Build the combined LLM + student corpus for one question."""
    rows = []
    ...
    for path in sorted(students_folder.iterdir(), key=lambda p: p.name.lower()):
        ...
        texts = responses.get(rnum, [])
        anon_id = anon_map.get(path.name, path.stem)

        for j, text in enumerate(texts, start=1):
            rows.append({
                "type": "STUDENT",
                "group": "Students",
                "id": f"{anon_id}#{j}" if len(texts) > 1 else anon_id,
                "text": text,
                "source_file": path.name,
            })
...
def compute_student_ai_scores(df: pd.DataFrame, X, rnum: int, top_k: int = 5) -> pd.DataFrame:
    """Compute student proximity metrics in the original TF-IDF space."""
    sim_all = cosine_similarity(X)
    type_upper = df["type"].str.upper()

    ia_mask = (type_upper == "IA").values
    st_mask = (type_upper == "STUDENT").values

    if ia_mask.sum() == 0 or st_mask.sum() == 0:
        raise ValueError(f"R{rnum}: missing LLM or student texts.")

    ia_indices = np.where(ia_mask)[0]
    ...
    for i in st_indices:
        ...
        best_ai_local = np.argmax(sims_ai)
        best_ai_idx = ia_indices[best_ai_local]
        ...
        mean_similarity_to_ai = float(np.mean(sims[ia_indices])) if len(ia_indices) > 0 else np.nan
        mean_similarity_to_students = float(np.mean(sims[other_students])) if len(other_students) > 0 else np.nan
...
def build_ranking_box_text(scores: pd.DataFrame, top_n: int = 21) -> str:
    """Build the text shown in the ranking box on the figure."""
    if scores.empty:
...
def compute_student_marker_sizes(
    ...
    """Assign student marker sizes based on best LLM similarity."""
    size_map = {row["student_id"]: row["best_similarity_to_ai"] for _, row in scores.iterrows()}
    sims = []

    for _, row in df_plot.iterrows():
        if str(row["type"]).upper() == "STUDENT":
            sims.append(float(size_map.get(row["id"], 0.0)))
        else:
            sims.append(np.nan)
    ...
    if len(valid) == 0:
        return np.full(len(df_plot), size_min)
...
def style_axis(ax):
    """Apply consistent styling for a multi-panel figure."""
...
def analyze_question(df: pd.DataFrame, rnum: int, out_dir: Path, top_k: int) -> Dict:
    """Compute TF-IDF, cosine similarity, t-SNE coordinates, and student scores."""
    if len(df) < 5:
        raise ValueError(f"Not enough texts for R{rnum} (need at least about 5).")
    ...
    scores = compute_student_ai_scores(df, X, rnum, top_k=top_k)
    ...
    student_sizes = compute_student_marker_sizes(df_plot, scores, STUDENT_SIZE_MIN, STUDENT_SIZE_MAX)
...
def plot_four_panels(results_by_question: Dict[int, Dict], out_dir: Path, show: bool = False):
    """Create a single 2x2 figure with questions R1..R4."""
    ...
    for ax, rnum in zip(axes, [1, 2, 3, 4]):
        ...
        ai_groups = df_plot[df_plot["type"] == "IA"]["group"].unique()
        ...
        for i in np.where(idx_students)[0]:
            ax.annotate(df_plot.loc[i, "id"], (Zx[i], Zy[i]), fontsize=ANNOTATION_FONTSIZE, alpha=0.97)
    ...
    for rnum in results_by_question:
        df_plot = results_by_question[rnum]["df_plot"]
        ai_groups = df_plot[df_plot["type"] == "IA"]["group"].unique()
        for group_name in ai_groups:
            label = f"LLM: {group_name}"
    ...
    print(f"  - {out_dir / 'Figure_all_questions_4panels.png'}")
    print(f"  - {out_dir / 'Figure_all_questions_4panels.pdf'}")


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Compare student exam answers with LLM-generated answers using TF-IDF, cosine similarity and t-SNE visualization."
    )
    parser.add_argument("--data-dir", type=Path, default=Path("data"), help="Directory containing LLM txt files.")
    parser.add_argument("--students-dir", type=Path, default=Path("data/students"), help="Directory containing student txt files.")
    parser.add_argument("--output-dir", type=Path, default=Path("outputs/output_10LLM"), help="Output directory.")
    parser.add_argument("--n-llm-responses", type=int, choices=[10, 20, 30], default=10, help="Use 10, 20 or 30 reference answers per LLM file.")
    parser.add_argument("--top-k", type=int, default=TOP_K_NEIGHBORS, help="Number of nearest neighbors used for the ai_in_topk metric.")
    parser.add_argument("--no-anonymization-map", action="store_true", help="Do not save the anonymization map CSV.")
    parser.add_argument("--show", action="store_true", help="Display the figure interactively in addition to saving it.")
...
def main():
    ...
    ia_files = build_ia_files(data_dir, args.n_llm_responses, DEFAULT_MODELS)
    anon_map, map_df = build_anonymization_map(
        students_folder=students_dir,
        output_dir=output_dir,
        save_map=not args.no_anonymization_map,
    )

    print("Anonymization table created.")
    ...
    for rnum in [1, 2, 3, 4]:
        print(f"\n=== Processing R{rnum} ===")
        df_r = build_R_corpus(ia_files, students_dir, rnum, anon_map)
        ...
        try:
            result_r = analyze_question(df_r, rnum, output_dir, top_k=args.top_k)
    ...
    if results_by_question:
        plot_four_panels(results_by_question, output_dir, show=args.show)
    ...
    if all_scores:
        ...
        print(f"\nCombined score file written: {output_dir / 'scores_all_questions.csv'}")
